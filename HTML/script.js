  ...
  if (window.isLoggedIn) {
    ...
    if (newQuantity <= 0) {
      ...
      fetch(window.BASE_URL + '/api/cart/update', {
        ...
        body: JSON.stringify({ product_id: item.id, size: item.size, quantity: newQuantity })
...
window.openModal = function(id) {
  ...
  if (!modalImgSrc.includes('w=')) {
    modalImgSrc += (modalImgSrc.includes('?') ? '&' : '?') + 'w=600&h=800&fit=crop';
  }
  
  document.getElementById('modalImg').innerHTML = `<img src="${modalImgSrc}" alt="${currentProduct.name}" onerror="handleImageError(this)">`;
...
// ══ PILLS – dùng GET method (URL param) ══
...
window.handleRegister = function(e) {
  ...
  if (!name || !email || !password) { showToast('Vui lòng điền đủ thông tin!'); return; }
  const params = new URLSearchParams(location.search);
...
async function initStore() {
  try {
    const res = await fetch(window.BASE_URL + '/api/products');
